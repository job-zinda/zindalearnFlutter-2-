...
//   // To store durations once loaded, to avoid flickering
...
//   // Add this to your _ChatRoomScreenState class
//   void _fetchDurationSilently(String msgId, String url) {
//     // Only fetch if we don't already have it
//     if (!_loadedDurations.containsKey(msgId)) {
//       final player = AudioPlayer();
//       player.setSourceUrl(url).then((_) {
...
//       if (mounted) CustomSnackbar.success(context, "Saved to Documents!");
//     } catch (e) {
//       if (mounted) {
...
//       isMe = msg["senderId"]["role"] == "student";
//     } else if (msg["senderId"] != null) {
//       isMe =
//           msg["isTemp"] == true ||
//           msg["senderId"].toString() == "6a27c046b84441e7fdf21acf";
//     } else if (msg["isTemp"] == true) {
//       isMe = true;
//     }
...
//                                 ? (Responsive.contentWidth(context) * 0.68)
//                                 : (Responsive.contentWidth(context) * 0.3),
//                             height: 190,
//                             decoration: BoxDecoration(
...
//                                       // 2. IMPORTANT: Listen for the duration and save it
//                                       audioPlayer.getDuration().then((d) {
//                                         if (d != null && mounted) {
//                                           setState(() {
//                                             _loadedDurations[msg["_id"]] =
//                                                 d; // Save the duration for this specific message
//                                           });
//                                         }
//                                       });
//                                       setState(
...
//                                                       )
//                                                     : (() {
//                                                         // This is a "silent" fetch triggered the moment the text renders
...
//                         if (mounted) CustomSnackbar.success(context, "Deleted");
//                       },
//                     ),
//                   ],
//                 ),
...
class ChatRoomScreen extends StatefulWidget {
  ...
  /// The requested/assigned tutor's known info at the time this screen was
  /// opened (from the rooms list or right after "Send Request"). This is
  /// only a starting snapshot — the room's live status (pending vs assigned)
  /// is read from the room/message data returned by ChatProvider once
  /// messages load, since that's the source of truth as the admin acts on
  /// the request.
  final Map<String, dynamic>? tutor;

 
  ///   context.read<AuthProvider>().userId
  /// Passing the wrong value here will misattribute every message.
...
class _ChatRoomScreenState extends State<ChatRoomScreen>
  ...
  // To store durations once loaded, to avoid flickering. Keyed consistently
  // by the same safe `msgId` string used for rendering (never raw msg["_id"]).
  final Map<String, Duration> _loadedDurations = {};
  // Tracks in-flight silent duration fetches so we don't spin up multiple
  // AudioPlayer instances for the same message while waiting on the first one.
  ...
  /// Fetch duration for a voice note without playing it, so we can show
  /// "00:24" style labels before the user taps play. Guards against
  /// duplicate concurrent fetches for the same message.
  // void _fetchDurationSilently(String msgId, String url) {
  ...
  /// Single source of truth for "is this message mine?" — used by both the
  /// normal message bubble and the tutor request card so behavior never
  ...
  /// Finds the most recent connect_card message in this room and reports
  /// whether the admin has assigned a tutor yet.
  ///
  /// ASSUMPTION (confirm against your backend): a connect card is
  /// "assigned" when either:
  ///   - card["assigned"] == true, or
  ///   - card["phone"] / card["email"] is present (contact info only
  ///     appears once the admin has actually assigned someone).
  /// Adjust the field names below if your API uses different keys.
  ...
  Widget buildNormalMessage(Map msg) {
    ...
    // Cap single-image bubble width on large screens so it doesn't blow up
    // to ~600+ px just because contentWidth is capped at 720/920.
    final double singleImageWidth =
        (Responsive.contentWidth(context) * 0.68).clamp(0.0, 380.0);
    final double multiImageWidth =
        (Responsive.contentWidth(context) * 0.3).clamp(0.0, 170.0);
  ...
  Widget buildTutorRequestCard(Map msg) {
    ...
    final bool isAssigned = _isCardAssigned(Map<String, dynamic>.from(card));
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            // STATUS BADGE — makes it obvious whether this is still an
            // admin-mediated request or an assigned, direct-chat tutor.
            Container(
            ...
            if (isAssigned && (phone.isNotEmpty || email.isNotEmpty)) ...[
              if (phone.isNotEmpty)
                Row(
                  children: [
                    Icon(Icons.phone, size: 14, color: AppColors.white.withAlpha(179)),
                    AppSpacing.w5,
                    Text(
                      phone,
                      style: AppTextStyles.body.copyWith(
              ...
              if (email.isNotEmpty) ...[
                ...
                Row(
                  children: [
                    Icon(Icons.email, size: 14, color: AppColors.white.withAlpha(179)),
                    AppSpacing.w5,
                    Text(
                      email,
                      style: AppTextStyles.body.copyWith(
  ...
  Widget build(BuildContext context) {
    ...
    // Derive live status from the room's own messages (source of truth)
    // rather than trusting the tutor snapshot passed in at navigation time,
    // since the admin can assign the tutor at any point while this screen
    // is open.
    final Map<String, dynamic>? latestCard =
        _latestConnectCard(provider.messages);
            ...
            children: [
              Text(
                ...
                isAssigned
                    ...
                    : (latestCard != null ? "Admin" : (headerTutorName ?? "Chat")),
